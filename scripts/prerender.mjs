...
function writeRedirects() {
  const lines = [
    ...
    "",
    "# Legacy duplicate of /contact kept working for old links. Safe to force:",
    "# no file is served at /contacts.",
...
// Deliberately absent: a forced `<route>  <route>/  301!` per page. Netlify's
// Pretty URLs already redirects the bare path to the trailing-slash one, and
// the forced flag applies a rule even when the path resolves to a file — so
// /services/ matched the /services rule and redirected to itself. That shipped
// and put every page into a redirect loop; do not reintroduce it.
